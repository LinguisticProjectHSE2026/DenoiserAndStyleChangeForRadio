...
`process(input_path: Path, output_base: Path): dict[str, list[Path]]`
...
`process(input_path: Path): AudioSegment`
...
`process(segment: AudioSegment): str`
...
`process(segment: AudioSegment): bool`
